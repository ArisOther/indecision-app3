...
var getDomainOfDataByKey = function getDomainOfDataByKey(data, key, type, filterNil) {
  ...
  if (type === 'number') {
    ...
    return domain.length ? [_lodash["default"].min(domain), _lodash["default"].max(domain)] : [Infinity, -Infinity];
  }
...
var calculateDomainOfTicks = function calculateDomainOfTicks(ticks, type) {
  if (type === 'number') {
    return [_lodash["default"].min(ticks), _lodash["default"].max(ticks)];
  }
...
var getCateCoordinateOfLine = function getCateCoordinateOfLine(_ref6) {
  ...
  var value = getValueByDataKey(entry, !_lodash["default"].isNil(dataKey) ? dataKey : axis.dataKey);
...
var getDomainOfSingle = function getDomainOfSingle(data) {
  return data.reduce(function (result, entry) {
    return [_lodash["default"].min(entry.concat([result[0]]).filter(_DataUtils.isNumber)), _lodash["default"].max(entry.concat([result[1]]).filter(_DataUtils.isNumber))];
  }, [Infinity, -Infinity]);
};
